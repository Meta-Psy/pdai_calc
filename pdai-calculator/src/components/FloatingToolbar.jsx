...
import { Download, Printer, Loader, Undo, RefreshCw } from './icons';
    ...
    <div className="fixed top-4 right-4 z-50 no-print animate-fade-in">
      <div className="flex items-center gap-1.5 bg-white/95 backdrop-blur-sm border border-gray-200 rounded-xl shadow-lg p-1.5">
        ...
        <button
          onClick={onUndo}
          disabled={!canUndo}
          className="p-2 text-gray-600 hover:bg-gray-100 disabled:opacity-30 rounded-lg transition-colors"
          title={t('actions.undo') + ' (Ctrl+Z)'}
        >
          <Undo />
        </button>

        {/* More menu (contains reset) */}
